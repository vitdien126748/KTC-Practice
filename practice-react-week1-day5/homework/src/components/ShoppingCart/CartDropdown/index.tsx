        ...
        <div className={styles.cartDropdown}>
                ...
                {cartItems.length === 0 ? (
                    ...
                    cartItems.map((item) => (
                        <div key={item.id} className={styles.cartItem}>
                            <span className={styles.itemName}><button className={styles.removeButton} onClick={() => cartContext.removeAllQuantityFromCart(item.id)}>X</button>{item.name}</span>
            ...
            {cartItems.length > 0 && (
                <div className={styles.totalPrice}>
                    <span>Tổng cộng:</span>
                    <span className={styles.totalAmount}>{cartItems.reduce((total, item) => total + (item.price * (item.quantity || 1)), 0).toLocaleString()}₫</span>
                </div>
            )}
